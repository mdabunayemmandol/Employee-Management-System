$(document).ready(function () {
    $("#txtFindTrainee").autocomplete({
        ...
        select: function (e, ui) {
            ...
            $.get("/api/GeneralInformations/EmploymentInfoAutoComplete", { id: ui.item.value })
                .done(function (data) {
...
$(document.body).on("click", "#btnSubmit", function () {
    ...
    vm.designation = $("#Designation").val();
    vm.overSeasExperience = $("#OverSeasExperience").val();
    vm.companyAddress = $("#CompanyAddress").val();
    vm.remark = $("#Remark").val();


    if (id == 0 || id == "" || id == null) {
        $.ajax({
            url: "/api/EmploymentInformations",
            data: vm,
            type: "POST",
            success: function (e) {
                if (e > 0) {
        ...
        vm.id = id;
        $.ajax({
            url: "/api/EmploymentInformations/" + id,
            data: vm,
            type: "PUT",
            success: function (e) {
                if (e > 0) {
...
function loadDatatable(id) {
    ...
    $("#EmploymentList").DataTable().destroy();

    $("#EmploymentList").DataTable({
        retrieve: true,
        paging: true,
        ajax: {
            url: "/api/EmploymentInformations/GetEmploymentInfo?id=" + id,
            dataSrc: ""
        },
        columns: [
            {
...
function getData(id) {
    $.get("/api/EmploymentInformations?id=" + id)
        .done(function (data) {
            $("#Id").val(id);
            $("#GeneralInformationId").val(data.generalInformationId);
            $("#CompanyName").val(data.companyName);
            $("#Designation").val(data.designation);
            $("#OverSeasExperience").val(data.overSeasExperience);
